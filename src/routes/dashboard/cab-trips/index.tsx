...
function CabTrips() {
  const { data: trips = [], isLoading, error } = useQuery(cabTripsQueryOptions);
  ...
  const tripStatusOptions = useMemo(() => {
    const set = new Set(trips.map((t) => t.trip_status).filter(Boolean));
    return Array.from(set) as string[];
  }, [trips]);

  const paymentStatusOptions = useMemo(() => {
    const set = new Set(trips.map((t) => t.payment_status).filter(Boolean));
    return Array.from(set) as string[];
  }, [trips]);
  ...
  const filtered = useMemo(() => {
    ...
    return trips.filter((t) => {
      if (q) {
        ...
        const phone = (t.client_phone ?? "").toLowerCase();
        if (!name.includes(q) && !phone.includes(q)) return false;
      }
      if (filterTripStatus !== "all") {
        if (
          (t.trip_status ?? "").toLowerCase() !== filterTripStatus.toLowerCase()
        )
          return false;
      }
      if (filterPaymentStatus !== "all") {
        if (
          (t.payment_status ?? "").toLowerCase() !==
  ...
  }, [trips, search, filterTripStatus, filterPaymentStatus]);
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Cab Trips</h1>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              ...
              {!isLoading &&
                !error &&
                paged.map((trip) => <TripRow key={trip.id} trip={trip} />)}
            </tbody>
          </table>
        </div>
...
function TripRow({ trip }: { trip: CabTrip }) {
  return (
    <tr className="hover:bg-gray-50/70 transition-colors group">
      {/* Customer */}
      <td className="px-5 py-3.5">
        <div className="flex items-center gap-3">
          <ClientAvatar src={trip.client_avatar} name={trip.client_name} />
      ...
      <td className="px-5 py-3.5 whitespace-nowrap">
        <span className="text-gray-700 font-medium">
          {formatDistance(trip.trip_total_distance)}
        </span>
      </td>
      ...
      {/* Pick Up Date */}
      <td className="px-5 py-3.5 whitespace-nowrap">
        <span className="text-gray-600">{trip.trip_date || "—"}</span>
      </td>

      {/* Pick Up Time */}
      <td className="px-5 py-3.5 whitespace-nowrap">
        <span className="text-gray-600">{formatTime(trip.trip_time)}</span>
      </td>
      ...
      {/* Payment Status */}
      <td className="px-5 py-3.5 whitespace-nowrap">
        <PaymentStatusBadge status={trip.payment_status} />
      </td>

      {/* Actions */}
      <td className="px-5 py-3.5 text-right whitespace-nowrap">
        <Link
          to="/dashboard/cab-trips/$tripId"
