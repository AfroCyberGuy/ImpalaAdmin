...
import {
  AssignDriverDialog,
  AssignSuccessToast,
} from "#/components/cab-trips/AssignDriverDialog";
import {
  APIProvider,
  Map,
  AdvancedMarker,
...
import {
  cabTripDetailQueryOptions,
  type CabTripDetail,
} from "#/utils/queries/cabTripQueries";

export const Route = createFileRoute("/dashboard/cab-trips/$tripId/")({
  component: ViewCabTrip,
});
...
function TripMap({ trip }: { trip: CabTripDetail }) {
...
// ── Airport Shuttle Panel ─────────────────────────────────────────────────────

function AirportShuttlePanel({ trip }: { trip: CabTripDetail }) {
  if (!trip.flight_passenger_name && !trip.flight_departure_arrival)
    return null;

  return (
    <SectionCard
      title="Airport Shuttle Details"
      icon={
        <svg
          className="w-4 h-4"
          fill="none"
    ...
    >
      <div className="py-1">
        <div className="mb-3 flex items-center gap-2 px-3 py-2.5 rounded-xl bg-amber-50 border border-amber-100">
          <svg
          ...
          <span className="text-xs font-semibold text-amber-700">
            Airport Shuttle Errand
          </span>
        </div>
        <InfoRow label="Passenger Name" value={trip.flight_passenger_name} />
        <InfoRow label="Contact Email" value={trip.flight_contact_email} />
        <InfoRow label="Cellphone" value={trip.flight_cellphone} />
        <InfoRow
          label="Departure / Arrival"
          value={trip.flight_departure_arrival}
        />
        <InfoRow label="Date" value={trip.flight_departure_arrival_date} />
        <InfoRow label="Time" value={trip.flight_departure_arrival_time} />
      </div>
    </SectionCard>
  );
}
...
function ViewCabTrip() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useQuery(cabTripDetailQueryOptions(Number(tripId)));

  const isAirportShuttle = trip?.errand_type?.toLowerCase().includes("airport");
  const canAssign =
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Trip #{trip.id}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Airport shuttle panel (conditional) */}
          {isAirportShuttle && <AirportShuttlePanel trip={trip} />}
        </div>
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <InfoRow
              ...
              value={
                trip.errand_type ? (
                  <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-violet-50 text-violet-700 ring-1 ring-violet-200">
                    {trip.errand_type}
                  </span>
                ) : null
              }
