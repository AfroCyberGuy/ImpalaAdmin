...
async function fetchCabTrips(): Promise<CabTrip[]> {
  const { data, error } = await supabase
    ...
    .select(
      `id, trip_date, trip_time, trip_cost, trip_total_distance,
       ...
       cab_rates(car_classifications(classification)),
       errand_types(name)`,
    )
  ...
  return (data ?? []).map((row) => {
    ...
    const cabRate = r.cab_rates as {
      car_classifications: { classification: string } | null;
    } | null;
    const errandType = r.errand_types as { name: string } | null;
    ...
    return {
      ...
      vehicle_type: cabRate?.car_classifications?.classification ?? null,
      errand_type: errandType?.name ?? null,
    };
  });
}

export const cabTripsQueryOptions = queryOptions({
  queryKey: ["cab-trips"],
  queryFn: fetchCabTrips,
});

// ── Detail ─────────────────────────────────────────────────────────────────────

export interface CabTripDetail extends CabTrip {
  ...
  // Flight details (airport shuttle)
  flight_passenger_name: string | null;
  flight_contact_email: string | null;
  flight_cellphone: string | null;
  flight_departure_arrival: string | null;
  flight_departure_arrival_date: string | null;
  flight_departure_arrival_time: string | null;
}

async function fetchCabTripById(id: number): Promise<CabTripDetail> {
  const { data, error } = await supabase
    .from("taxi_bookings")
    .select(
      `id, trip_date, trip_time, trip_cost, trip_total_distance,
       ...
       flight_details(name, contact_email, cellphone, departure_arrival, departure_arrival_date, departure_arrival_time)`,
    )
    .eq("id", id)
    .single();
  ...
  const flight = r.flight_details as {
    name: string | null;
    contact_email: string | null;
    cellphone: string | null;
    departure_arrival: string | null;
    departure_arrival_date: string | null;
  ...
  return {
    ...
    flight_passenger_name: flight?.name ?? null,
    flight_contact_email: flight?.contact_email ?? null,
    flight_cellphone: flight?.cellphone ?? null,
    flight_departure_arrival: flight?.departure_arrival ?? null,
    flight_departure_arrival_date: flight?.departure_arrival_date ?? null,
    flight_departure_arrival_time: flight?.departure_arrival_time ?? null,
  };
}

export function cabTripDetailQueryOptions(id: number) {
  return queryOptions({
    queryKey: ["cab-trip", id],
    queryFn: () => fetchCabTripById(id),
  });
}
