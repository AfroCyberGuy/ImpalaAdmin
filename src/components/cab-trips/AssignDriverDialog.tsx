...
import { assignDriverToTrip } from "#/utils/queries/cabTripQueries";
...
// ── Step indicator shown inside the dialog while assigning ────────────────────
...
export function AssignDriverDialog({
  ...
  const mutation = useMutation({
    mutationFn: ({ driverId }: { driverId: number }) => {
      // Simulate two-phase UX: assigning → notifying happen inside the edge fn,
      // but we advance the step indicator after a short delay so the user sees progress.
      setStep("assigning");
      return new Promise<{ driverId: number }>((resolve, reject) => {
        // After 600ms visually move to "notifying" step, then actually invoke
    ...
    onSuccess: (_data, { driverId }) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["cab-trips"] });
      queryClient.invalidateQueries({ queryKey: ["cab-trip", tripId] });
  ...
  // Close on Escape (disabled while assigning)
  useEffect(() => {
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100 shrink-0">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Trip #{tripId} · select a driver to assign
            </p>
          </div>
          <button
            onClick={onClose}
